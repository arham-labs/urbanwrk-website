        ...
        <div className="flex flex-col gap-10">
            <div className="flex items-center whitespace-nowrap">
                <div className="flex overflow-auto overflow-x-scroll scrollbar-hide w-full">
                    {titles.map((item, i) =>
                        <a className="text-sm lg:text-base uppercase font-medium w-full lg:w-1/3 text-center border-b border-[#747474] pb-6 lg:pb-4 cursor-pointer relative" key={i} onClick={() => onClickItem(item.id)}>
            ...
            {showData.map((item, i) =>
                <div className="flex flex-col lg:flex-row items-end gap-10 lg:gap-24" key={i}>
                    ...
                    <div className="flex flex-col w-full lg:w-1/2">
                        ...
                        <a className="border border-black block w-fit py-2 px-4 text-sm">{item.btn}</a>
                    </div>
                </div>
            )}
        </div>)
