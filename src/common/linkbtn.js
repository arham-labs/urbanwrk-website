...
export default function LinkButton({ title, href, margin, marginspace, disabled }) {
  return (
    <div className={`group ${disabled}`}>
      <a href={href ? href : "/"} className={`text-base border-black border px-4 gap-2 flex items-center h-[36px] w-fit transition-all ease-in-out group-hover:bg-primary group-hover:border-primary group-hover:text-white cursor-pointer ${margin
        ? "lg:ml-[75px]"
        : marginspace
